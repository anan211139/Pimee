...
class graph_subjectController extends Controller
{
    public function main($id){
        ...
        $jsonchapters = DB::table('chapters')->where('subject_id','=',$subject_id)->get();
        ...
        $listoverallscore_sub1 = DB::table(
          DB::raw("(select sum(level_id * total_level_true) as score, groups.line_code from results 
          left join groups on results.group_id = groups.id
          left join chapters on groups.chapter_id = chapters.id
          where subject_id = $subject_id and chapter_id = 1
          group by group_id) temp"))
          ->select(DB::raw('sum(temp.score) / count(temp.line_code) as score,line_code'))
          ->groupby('temp.line_code')
          ->get();
        // return $listoverallscore_sub1;
        $listoverallscore_sub2 = DB::table(
          DB::raw("(select sum(level_id * total_level_true) as score, groups.line_code from results 
          left join groups on results.group_id = groups.id
          left join chapters on groups.chapter_id = chapters.id
          where subject_id = $subject_id and chapter_id = 2
          group by group_id) temp"))
          ->select(DB::raw('sum(temp.score) / count(temp.line_code) as score,line_code'))
          ->groupby('temp.line_code')
          ->get();
        // return $listoverallscore_sub2;
        ...
        Session::put('listoverallscore_sub1',$listoverallscore_sub1);
        Session::put('listoverallscore_sub2',$listoverallscore_sub2);
